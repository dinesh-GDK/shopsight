...
**File:** `src/components/search/ConfidenceSlider.jsx`

**Purpose:** Interactive slider to filter search results by confidence score threshold

**Props:**
- `value: number` - Current confidence value (0.0 - 1.0)
- `onChange: (value: number) => void` - Callback when slider value changes
- `disabled: boolean` - Whether slider is disabled during loading

**Design Specs:**
- Width: Full width (max 600px centered)
- Height: Adaptive
- Slider track: Blue gradient showing current threshold
- Slider thumb: 20px circle with white border
- Labels: Dynamic based on threshold level
- Help text: Shows current filter message

**Features:**
- Default value: 0.5 (50% confidence)
- Real-time filtering as user drags
- Visual feedback with color-coded labels:
  - 0-30%: Gray - "Show All"
  - 30-50%: Orange - "Low Match"
  - 50-80%: Yellow - "Medium Match"
    ...
    <>
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-4">
        ...
        <input
          ...
          disabled={disabled}
          className="w-full h-2 bg-gray-200 rounded-lg confidence-slider"
          style={{
            background: `linear-gradient(to right,
              #3b82f6 0%, #3b82f6 ${value * 100}%,
              #e5e7eb ${value * 100}%, #e5e7eb 100%)`
          }}
        />
...
2. Slider appears with default 50% threshold
3. User sees results filtered to 50%+ confidence
4. User drags slider left to see more results (lower confidence)
5. User drags slider right to see only best matches (higher confidence)
6. Results update in real-time, pagination resets to page 1
...
**Purpose:** Display individual product with image, metadata, and confidence score
...
```typescript
{
  product: {
    ...
    department: string;
    image_url: string;
    confidence_score: number; // 0.0 - 1.0 (NEW)
  }
}
```
...
- Confidence Badge: Top-right corner, color-coded by score
  - 80-100%: Green badge (Excellent match)
  - 60-80%: Blue badge (Good match)
  - 40-60%: Yellow badge (Fair match)
  - 0-40%: Gray badge (Low match)
